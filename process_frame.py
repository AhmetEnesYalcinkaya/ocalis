    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                # # ------------------- Line --------------
                cv2.line(frame, left_shldr_coord, right_shldr_coord, self.COLORS['green'], 4, lineType=self.linetype) #11-12
                cv2.line(frame, right_shldr_coord, right_elbow_coord, self.COLORS['green'], 4, lineType=self.linetype) #12-14
                cv2.line(frame, right_elbow_coord, right_wrist_coord, self.COLORS['green'], 4, lineType=self.linetype) #14-16
                cv2.line(frame, left_shldr_coord, left_elbow_coord, self.COLORS['green'], 4, lineType=self.linetype) #11-13
                cv2.line(frame, left_elbow_coord, left_wrist_coord, self.COLORS['green'], 4, lineType=self.linetype) #13-15
                cv2.line(frame, right_shldr_coord, right_hip_coord, self.COLORS['green'], 4, lineType=self.linetype) #12-24
                cv2.line(frame, left_shldr_coord, left_hip_coord, self.COLORS['green'], 4, lineType=self.linetype) #11-23
                cv2.line(frame, left_hip_coord, right_hip_coord, self.COLORS['green'], 4, lineType=self.linetype) #23-24
                cv2.line(frame, right_hip_coord, right_knee_coord, self.COLORS['green'], 4, lineType=self.linetype) #24-26
                cv2.line(frame, right_knee_coord, right_ankle_coord, self.COLORS['green'], 4, lineType=self.linetype) #24-26
                cv2.line(frame, left_hip_coord, left_knee_coord, self.COLORS['green'], 4, lineType=self.linetype) #23-25
                cv2.line(frame, left_knee_coord, left_ankle_coord, self.COLORS['green'], 4, lineType=self.linetype) #25-27
                # ------------------------------------------
                ...
                draw_text(
                    frame, 
                    'POSTUR ANALIZI ICIN YAN DURUNUZ!', 
                    pos = (25,15),
            ...
            # İnsan yan görünümde 
                ...
                cv2.ellipse(frame, ankle_coord, (20, 20),
                ...
                cv2.ellipse(frame, shldr_coord, (20, 20), 
                            angle = 180, startAngle = -90, endAngle = -90 - multiplier*elbow_vertical_angle, 
                            color = self.COLORS['white'], thickness = 3, lineType = self.linetype)
                ...
                cv2.line(frame, wrist_coord, elbow_coord, self.COLORS['green'], 4, lineType=self.linetype)

                # Gövde analizi
                if hip_vertical_angle >= 0 and hip_vertical_angle <5:
                    cv2.line(frame, shldr_coord, hip_coord, self.COLORS['green'], 4, lineType=self.linetype)
                elif hip_vertical_angle > 5 and hip_vertical_angle < 20:
                    cv2.line(frame, shldr_coord, hip_coord, self.COLORS['yellow'], 4, lineType=self.linetype)
                elif hip_vertical_angle > 20 and hip_vertical_angle < 60:
                    cv2.line(frame, shldr_coord, hip_coord, self.COLORS['orange'], 4, lineType=self.linetype)
                else:
                    cv2.line(frame, shldr_coord, hip_coord, self.COLORS['red'], 4, lineType=self.linetype)
                
                #Bacak analizi
                if knee_vertical_angle >= 0 and knee_vertical_angle < 5:
                ...
                elif knee_vertical_angle > 5 and knee_vertical_angle < 30:
                    cv2.line(frame, knee_coord, hip_coord, self.COLORS['yellow'], 4,  lineType=self.linetype)
                    cv2.line(frame, ankle_coord, knee_coord,self.COLORS['yellow'], 4,  lineType=self.linetype)
                elif knee_vertical_angle > 30 and knee_vertical_angle < 60:
                    cv2.line(frame, knee_coord, hip_coord, self.COLORS['orange'], 4,  lineType=self.linetype)
                    cv2.line(frame, ankle_coord, knee_coord,self.COLORS['orange'], 4,  lineType=self.linetype)
                else:
                    cv2.line(frame, knee_coord, hip_coord, self.COLORS['red'], 4,  lineType=self.linetype)
                    cv2.line(frame, ankle_coord, knee_coord,self.COLORS['red'], 4,  lineType=self.linetype)
                
                # Üst Kollar

                if elbow_vertical_angle >= 0 and elbow_vertical_angle < 20:
                    cv2.line(frame, shldr_coord, elbow_coord, self.COLORS['green'], 4,  lineType=self.linetype)
                    cv2.line(frame, elbow_coord, wrist_coord,self.COLORS['green'], 4,  lineType=self.linetype)
                elif elbow_vertical_angle > 20 and elbow_vertical_angle < 45:
                    cv2.line(frame, shldr_coord, elbow_coord, self.COLORS['yellow'], 4,  lineType=self.linetype)
                    cv2.line(frame, elbow_coord, wrist_coord,self.COLORS['yellow'], 4,  lineType=self.linetype)
                elif elbow_vertical_angle > 45 and elbow_vertical_angle < 90:
                    cv2.line(frame, shldr_coord, elbow_coord, self.COLORS['orange'], 4,  lineType=self.linetype)
                    cv2.line(frame, elbow_coord, wrist_coord,self.COLORS['orange'], 4,  lineType=self.linetype)
                else:
                    cv2.line(frame, shldr_coord, elbow_coord, self.COLORS['red'], 4,  lineType=self.linetype)
                    cv2.line(frame, elbow_coord, wrist_coord,self.COLORS['red'], 4,  lineType=self.linetype)
                ...
                cv2.putText(frame, str(int(elbow_vertical_angle)), (shldr_text_coord_x, shldr_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
